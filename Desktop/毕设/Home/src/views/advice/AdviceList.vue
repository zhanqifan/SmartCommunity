<script setup>
import { ComplainList,delMessage } from "@/api/bill-manage";
...
// 获取列表
const GetList = async () => {
  const data = User.UserInfo.role==1?{userId:User.UserInfo._id}:{}
...
const handlepage =async(page) =>{
   const data = User.UserInfo.role==1?{userId:User.UserInfo._id,...page}:page
     const res =await ComplainList(data)
     tableData.value = res.data
     count.value = res.total
