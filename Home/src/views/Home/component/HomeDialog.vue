...
const avatarUrl = computed(() => {
  return useUsers.UserInfo.avatar
    ? useUsers.UserInfo.avatar
...
const submitForm = () => {
  ruleFormRef.value.validate(async (valid) => {
    if (valid) {
      ...
      useUsers.changeUserInfo(res.data);
    } else {
      ElMessage({
        message: "更新失败",
        type: "error",
...
onMounted(() => {
  ruleForm.value = JSON.parse(JSON.stringify(useUsers.UserInfo)); //利用深拷贝 为了不影响pinia里数据
  console.log(ruleForm.value)
});
</script>
<template>
    ...
    <el-row>
      <el-col :span="4">
        <el-card class="box-card1" shadow="always">
          ...
          <p>{{ useUsers.UserInfo.username }}</p>
          <h3>{{ useUsers.UserInfo.role===0 ? "管理员" : "用户" }}</h3>
        </el-card>
      </el-col>
