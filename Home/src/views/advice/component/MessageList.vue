...
import { PostMessage } from "@/api/bill-manage";
...
const isCurrentUser = (userId) => {
  return userId === User.UserInfo._id;
};
...
const SendMessage = reactive({
  textarea: "", //发送信息文本
  userId: mainId, //反馈集合关联的用户 用于查找是哪一天反馈信息
  commentId: User.UserInfo._id, //发送评论的用户id
});
...
const PostMsgList = async () => {
  ...
  messages.value = messages.value = res.data.filter(item=>item._id==SendMessage.userId)[0].comments
